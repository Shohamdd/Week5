...
function addSelection(data, identifier) {
  let CurrenciesC = Object.keys(data).filter((v) => v != null);
  let identifierC = identifier.filter((v) => v.AlphabeticCode != null);
  let selectOption = CurrenciesC.map((v) => {
    let index = identifierC.findIndex((x) => {
      if (`USD${x.AlphabeticCode}` === v) return true;
    });
    if (index >= 0) {
      return [identifierC[index].Currency, v];
    }
  });
